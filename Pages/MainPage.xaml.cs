...
namespace CatApp
{
    public partial class MainPage : ContentPage
    {
        ...
        async void AddCats(int amount) {
            isLoading = true;
            List<Cat>? cats = await TheCatAPIService.Current.GetCatsAsync(amount, false, Size.Thumb);

            foreach (Cat cat in cats!) {
                InfiniteScrollContent.Add(new CatItem(cat.id!, cat.url!, cat.width, cat.height));
            }
            isLoading = false;
        }
